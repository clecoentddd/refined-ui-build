...
export default function RadarElementCard({ element: el, onEdit, onDelete }: Props) {
  return (
    <div className="bg-card border border-border rounded-xl p-4 hover:border-foreground/15 transition-all group">
      <div className="flex items-start justify-between mb-1.5">
        <div className="font-bold text-sm leading-snug">{el.title}</div>
        <Pill variant={TB[el.type] || 'default'}>{el.type}</Pill>
      </div>
      <div className="font-mono text-[10px] text-muted-foreground mb-2.5">{(el.category || '').replace(/_/g, ' ')}</div>
      ...
      {el.assess && <div className="font-mono text-[10px] text-muted-foreground leading-relaxed mb-3">{el.assess}</div>}
      <div className="flex gap-1.5">
        <button onClick={onEdit} className="inline-flex items-center gap-1 border border-border bg-card text-muted-foreground hover:text-foreground hover:border-foreground/20 px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all">
