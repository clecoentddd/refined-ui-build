  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
                ...
                <>
                  ...
                  <div className="mb-5 rounded-lg border border-border bg-muted/20 divide-y divide-border overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 px-3 py-2 min-h-[36px]">
                      ...
                      <div className="flex flex-wrap gap-1.5 min-w-0">
                        {initiatives.filter(i => linkedInitiativeIds.has(i.initiativeId)).map(i => (
      ...
      <Dialog open={iLinksModalOpen} onOpenChange={v => !v && setILinksModalOpen(false)}>
        <DialogContent className="max-w-lg w-full bg-card border-border rounded-2xl p-0 overflow-hidden">
          <DialogHeader className="px-6 pt-6 pb-4 border-b border-border">
            <div className="flex items-center justify-between">
          ...
          <div className="p-4 flex flex-col gap-1 max-h-[50vh] overflow-y-auto">
              ...
              .map(i => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2.5 w-full text-left px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${selected ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:bg-muted/50 hover:text-foreground'
                      }`}
                  >
            ...
            {initiatives.filter(i => i.initiativeId !== selectedInitiative?.initiativeId).length === 0 && (
              <p className="text-xs text-muted-foreground/40 italic p-2">No other initiatives found in this strategy.</p>
            )}
          </div>
          ...
          <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-border">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-5 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-semibold hover:opacity-90 transition-all disabled:opacity-50"
            >
              {iLinksSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
